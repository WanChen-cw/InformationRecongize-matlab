function polar_info_esti_all = SCL_decoder(llr, L, Sd)%2018.1.7.14:16 Yu Y. R.
...
%         compare = PM_pair <= PM_cv; 
        ...
        for l_index = 1 : L
            ...
            if (P(1, l_index) < 0) &&(polar_info_esti_all(phi+1)==0)
                PM(l_index) = PM(l_index) - P(1, l_index);
            end
            if (P(1, l_index) > 0) &&(polar_info_esti_all(phi+1)==1)
                PM(l_index) = PM(l_index) + P(1, l_index);
            end
            if phi_mod_2 == 0
                C(1, 2 * l_index - 1) = polar_info_esti_all(phi+1);
            else
                C(1, 2 * l_index) = polar_info_esti_all(phi+1);
            end 
        end
    end 
